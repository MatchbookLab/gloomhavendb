...
_.forEach(controllers, (controllerPath, className) => {
  ...
  controller.getStructure().methods.forEach((method: MethodDeclarationStructure) => {
    ...
    impl += `    return (await fetch(\`${url}\`, { method: '${httpMethod}'${body}, headers: { 'Content-Type': 'application/json' } })).json();\n`;
    impl += `  }\n`;
...
const entityFiles = fs.readdirSync(path.join(__dirname, '../shared/entities/'));
